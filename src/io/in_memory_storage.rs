...
mod tests {
    ...
    fn test_delete() {
        ...
        let result = storage.delete_data(test_path).expect("Failed to delete");
        assert!(result);

        // Delete non-existent should return false
        let result2 = storage.delete_data(test_path).expect("Failed to delete");
        assert!(!result2);
    }
    ...
    fn test_overwrite() {
        ...
        let loaded = storage.load_data(test_path).expect("Failed to load");
    ...
    fn test_nested_paths() {
        ...
        let loaded = storage.load_data(nested_path).expect("Failed to load");
        assert_eq!(b"nested".to_vec(), loaded);
    }

    #[test]
    fn test_multiple_storages_independent() {
        ...
        let data1 = storage1.load_data(test_path).expect("Failed to load");
        let data2 = storage2.load_data(test_path).expect("Failed to load");
